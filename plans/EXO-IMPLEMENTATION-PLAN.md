...
| --- | --- | --- | --- | --- | --- |
| RT | CO / `basilica-backend-exo` | Section 4 runtime paths | Pinned runtime, persistence, physical restart/capture, bounded transfer and private durable publication; checkpoints remain required | Private S3 publication and forced-store-restart verification implemented; hosted capture-to-storage acceptance, export resume, owner retrieval and retention pending | `6e8d8043`; 899 API tests, 65 owned delivery cases including 10 storage fault tests, 26 schema cases and strict Clippy pass; native disk-backed S3 verifies both objects after forced restart and denies unsigned access; earlier physical 1.16 GB capture/transfer evidence is recorded at `21d845053`; storage head `cdafe238` passed CI35567107848; final-head CI35568524093 is tracked in backend PR 1872 |
| LC | CO / `basilica-backend-exo` | Lifecycle/catalog, allocation/authority, billing/cleanup/archival, protected delivery, create/delete/restart/readiness and private export key/transfer/storage authority; API migrations 035–047 and billing migrations 048–049 | G0; G1 for runtime adapter | Internal coordinators, guarded Ready, physical fencing, rotation, retained export keys and atomic verified-artifact registration implemented; service worker, owner retrieval/retention, export/recovery and unpaid-tail/preservation policy pending | `6e8d8043`; 200 API integration cases and 65 owned delivery cases pass, including uncertainty, corruption, takeover and atomic storage-registration rollback; native object-store restart/private-access test passes; 899 API tests, strict Clippy and full 79-commit secret scan pass; 31 allocator and 30 billing cases pass after a reproduced concurrent-hostname fix; storage head `cdafe238` passed CI35567107848; final-head CI35568524093 is tracked in backend PR 1872 |
