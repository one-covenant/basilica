...
| --- | --- | --- | --- | --- | --- |
| RT | CO / `basilica-backend-exo` | Section 4 runtime paths | Pinned runtime, persistence, physical restart, retained-key capture and authorized bounded transfer; durable storage and checkpoints remain required | Complete owned controller/OpenSSH/Engine transfer verified; durable artifacts, export resume/coordinator, account retrieval and hosted acceptance pending | `21d845053`; actual 1,159,167,648-byte payload and 839-byte metadata transfer with whole hashes, retirement and preserved state pass in 505.09s; 898 API tests, 55 owned delivery cases, 82 host cases and strict Clippy pass; predecessor `8f6cd726` passed CI35559655254; exact-head CI tracked in backend PR 1872 |
| LC | CO / `basilica-backend-exo` | Lifecycle/catalog, allocation/authority, billing/cleanup/archival, protected delivery, create/delete/restart/readiness and private export-key/transfer authority; API migrations 035–046 and billing migrations 048–049 | G0; G1 for runtime adapter | Internal coordinators, guarded Ready, physical fencing, startup rotation, durable export keys and fixed captured-object reads implemented; service worker, durable artifact publication/retrieval, export/recovery and unpaid-tail/preservation policy pending | `21d845053`; 55 owned delivery cases include read-authority loss at SSH boundaries; 898 API tests (65 expected ignores), actual gigabyte transfer, strict Clippy and full 77-commit secret scan pass; earlier broad integration evidence and current exact-head CI tracked in backend PR 1872 |
