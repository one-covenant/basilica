...
impl AuthenticatedRequest for protocol::executor_control::ProvisionAccessRequest {
    fn get_auth(&self) -> Option<&MinerAuthentication> {
        self.auth.as_ref()
    }
}

impl AuthenticatedRequest for protocol::executor_control::SystemProfileRequest {
    fn get_auth(&self) -> Option<&MinerAuthentication> {
        self.auth.as_ref()
    }
}

impl AuthenticatedRequest for protocol::executor_control::BenchmarkRequest {
    fn get_auth(&self) -> Option<&MinerAuthentication> {
        self.auth.as_ref()
    }
}

impl AuthenticatedRequest for protocol::executor_control::ContainerOpRequest {
    fn get_auth(&self) -> Option<&MinerAuthentication> {
        self.auth.as_ref()
    }
}

impl AuthenticatedRequest for protocol::executor_control::HealthCheckRequest {
    fn get_auth(&self) -> Option<&MinerAuthentication> {
        self.auth.as_ref()
    }
}
...
mod tests {
    ...
    async fn test_verify_miner_request_helper() {
        ...
        // Test without auth
        let request_no_auth = protocol::executor_control::ProvisionAccessRequest {
            validator_hotkey: "validator".to_string(),
            ssh_public_key: "ssh-rsa ...".to_string(),
            access_token: String::new(),
            duration_seconds: 3600,
