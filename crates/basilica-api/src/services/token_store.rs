...
use crate::models::auth::{AuthError, TokenSet};
...
const SERVICE_NAME: &str = "basilica";
...
impl TokenStore {
    ...
    pub fn new() -> Result<Self, AuthError> {
        let project_dirs =
            ProjectDirs::from("ai", "basilica", "basilica").ok_or_else(|| {
    ...
    pub fn with_config(use_keychain: bool, storage_path: Option<&Path>) -> Result<Self, AuthError> {
    ...
    pub async fn store_tokens(&self, service_name: &str, tokens: &TokenSet) -> Result<(), AuthError> {
    ...
    pub async fn store(&self, service_name: &str, tokens: &TokenSet) -> Result<(), AuthError> {
        self.store_tokens(service_name, tokens).await
    }

    /// Retrieve stored tokens
    pub async fn get_tokens(&self, service_name: &str) -> Result<Option<TokenSet>, AuthError> {
        if self.use_system_keychain {
    ...
    pub async fn retrieve(&self, service_name: &str) -> Result<Option<TokenSet>, AuthError> {
        self.get_tokens(service_name).await
    }

    /// Delete stored tokens
    pub async fn delete_tokens(&self, service_name: &str) -> Result<(), AuthError> {
        if self.use_system_keychain {
    ...
    pub async fn delete(&self, service_name: &str) -> Result<(), AuthError> {
        self.delete_tokens(service_name).await
    }

    /// Check if tokens exist for a service
    pub async fn has_tokens(&self, service_name: &str) -> Result<bool, AuthError> {
    ...
    pub async fn update_tokens(&self, service_name: &str, tokens: &TokenSet) -> Result<(), AuthError> {
        // For atomic update, we simply overwrite the existing tokens
        self.store_tokens(service_name, tokens).await
    }

    /// List all stored service names
    pub async fn list_services(&self) -> Result<Vec<String>, AuthError> {
    ...
    async fn store_in_keychain(&self, service_name: &str, tokens: &TokenSet) -> Result<(), AuthError> {
        let _entry = self
            .keyring_entry
            .as_ref()
    ...
    async fn retrieve_from_keychain(&self, service_name: &str) -> Result<Option<TokenSet>, AuthError> {
        let metadata = self.load_metadata()?;
    ...
    async fn delete_from_keychain(&self, service_name: &str) -> Result<(), AuthError> {
        let metadata = self.load_metadata()?;
    ...
    async fn store_in_file(&self, service_name: &str, tokens: &TokenSet) -> Result<(), AuthError> {
    ...
    async fn retrieve_from_file(&self, service_name: &str) -> Result<Option<TokenSet>, AuthError> {
        let storage_path = self
            .storage_path
            .as_ref()
            .ok_or_else(|| AuthError::StorageError("Storage path not configured".to_string()))?;
    ...
    async fn delete_from_file(&self, service_name: &str) -> Result<(), AuthError> {
        let storage_path = self
            .storage_path
            .as_ref()
            .ok_or_else(|| AuthError::StorageError("Storage path not configured".to_string()))?;
    ...
    fn load_metadata(&self) -> Result<TokenMetadata, AuthError> {
    ...
    fn save_metadata(&self, metadata: &TokenMetadata) -> Result<(), AuthError> {
    ...
    fn update_metadata_for_service(&self, service_name: &str, storage_key: &str) -> Result<(), AuthError> {
    ...
    fn remove_metadata_for_service(&self, service_name: &str) -> Result<(), AuthError> {
        let mut metadata = self.load_metadata()?;
