...
impl SshService for DefaultSshService {
    ...
    async fn close_tunnel(&self, tunnel_id: &str) -> Result<(), SshError> {
        if let Some((_, mut child)) = self.tunnels.remove(tunnel_id) {
            child.kill()
                .map_err(|e| SshError::ConnectionFailed(format!("Failed to close tunnel: {}", e)))?;
        }
...
impl SshService for MockSshService {
    async fn generate_key_pair(&self, key_type: Option<&str>) -> Result<SshKeyPair, SshError> {
        ...
        self.generated_keys.lock().insert(key_id.clone(), key_pair.clone());
    ...
    async fn create_tunnel(
        ...
        // Store tunnel info
        self.tunnels.lock().insert(tunnel_id.clone(), forwards.clone());
...
mod tests {
    ...
    async fn test_validate_public_key() {
        ...
        let valid_key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOMock1234567890abcdefghijklmnop test@example.com";
    ...
    async fn test_create_tunnel() {
        ...
        let tunnel_id = service.create_tunnel(
            &access,
            forwards,
            Path::new("/tmp/test_key")
        ).await.unwrap();
    ...
    async fn test_multiple_tunnels() {
        ...
        let tunnel1 = service.create_tunnel(
            &access,
            vec![PortForward {
                local_port: 8080,
                remote_host: "localhost".to_string(),
        ...
        let tunnel2 = service.create_tunnel(
            &access,
            vec![PortForward {
                local_port: 3306,
                remote_host: "db.internal".to_string(),
