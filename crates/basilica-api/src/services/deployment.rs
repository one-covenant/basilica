...
use crate::models::deployment::{Deployment, DeploymentStatus, DeploymentValidationError};
use crate::models::executor::{ExecutorFilters, ResourceRequirements};
...
impl DefaultDeploymentService {
    ...
    async fn validate_request(
        ...
        if duration > self.config.max_duration_hours {
            return Err(DeploymentError::ResourceLimitExceeded(format!(
                "Duration {} hours exceeds maximum {} hours",
                duration, self.config.max_duration_hours
            )));
        }
...
impl DeploymentService for DefaultDeploymentService {
    async fn create_deployment(
        ...
        if !token.scopes.is_empty()
            && !token
                .scopes
                .iter()
                .any(|s| s == "deployments:create" || s == "compute:write" || s == "*")
        {
    ...
    async fn update_deployment_status(
        ...
        // Update in store
        self.deployments
            .insert(deployment_id.to_string(), deployment.clone());
    ...
    async fn extend_deployment(
        ...
        if let Some(expires_at) = deployment.expires_at {
            ...
            if total_duration.num_hours() > self.config.max_duration_hours as i64 {
                return Err(DeploymentError::ResourceLimitExceeded(format!(
                    "Extension would exceed maximum duration of {} hours",
...
impl DeploymentService for MockDeploymentService {
    async fn create_deployment(
        ...
        let deployment = Deployment {
            ...
            executor_id: request
                .executor_id
                .unwrap_or_else(|| "exec-test".to_string()),
            image: request.image,
    ...
    async fn extend_deployment(
        ...
        if let Some(expires_at) = deployment.expires_at {
            deployment.expires_at =
                Some(expires_at + chrono::Duration::hours(additional_hours as i64));
            deployment.updated_at = Utc::now();
        }
...
mod tests {
    ...
    fn create_test_request() -> CreateDeploymentRequest {
        CreateDeploymentRequest {
            image: "ubuntu:latest".to_string(),
            ssh_public_key:
                "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOMockKeyDataHere test@example.com".to_string(),
    ...
    async fn test_list_deployments() {
        ...
        service
            .create_deployment(request.clone(), "user123")
            .await
            .unwrap();
        service
            .create_deployment(request.clone(), "user123")
    ...
    async fn test_list_deployments_with_filter() {
        ...
        service
            .terminate_deployment(&response.deployment_id, None)
            .await
            .unwrap();

        let running = service
            .list_deployments("user123", Some(DeploymentStatus::Running))
            .await
            .unwrap();
        assert_eq!(running.len(), 0);

        let terminated = service
            .list_deployments("user123", Some(DeploymentStatus::Terminated))
            .await
            .unwrap();
    ...
    async fn test_update_deployment_status() {
        ...
        service
            .update_deployment_status(&response.deployment_id, DeploymentStatus::Stopping)
            .await
            .unwrap();
    ...
    async fn test_terminate_deployment() {
        ...
        service
            .terminate_deployment(&response.deployment_id, Some("User requested".to_string()))
            .await
            .unwrap();
    ...
    async fn test_get_deployment_details() {
        ...
        let response = service.create_deployment(request, "user123").await.unwrap();
        let details = service
            .get_deployment_details(&response.deployment_id)
            .await
            .unwrap();
    ...
    async fn test_get_deployment_logs() {
        ...
        let response = service.create_deployment(request, "user123").await.unwrap();
        let logs = service
            .get_deployment_logs(&response.deployment_id, None, false)
            .await
            .unwrap();
    ...
    async fn test_get_deployment_logs_with_tail() {
        ...
        let response = service.create_deployment(request, "user123").await.unwrap();
        let logs = service
            .get_deployment_logs(&response.deployment_id, Some(1), false)
            .await
            .unwrap();
    ...
    async fn test_get_deployment_metrics() {
        ...
        let response = service.create_deployment(request, "user123").await.unwrap();
        let metrics = service
            .get_deployment_metrics(&response.deployment_id)
            .await
            .unwrap();
    ...
    async fn test_extend_deployment() {
        ...
        service
            .extend_deployment(&response.deployment_id, 24)
            .await
            .unwrap();
        ...
        assert_eq!(
            new_expiry
                .signed_duration_since(original_expiry)
                .num_hours(),
            24
        );
    }
    ...
    async fn test_validate_excessive_duration() {
        ...
        assert!(matches!(
            result,
            Err(DeploymentError::ResourceLimitExceeded(_))
        ));
    }
}
