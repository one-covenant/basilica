...
mod tests {
    ...
    async fn test_file_cache_storage_persistence() {
        ...
        
        {
            ...
            cache.set("persist_key", &"persist_value", None).await.unwrap();
        }
    ...
    async fn test_file_cache_storage_concurrent_access() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                cache_clone.set(&format!("key{}", i), &format!("value{}", i), None).await.unwrap();
            });
            handles.push(handle);
        }
    ...
    async fn test_cache_service_stats() {
        ...
        // Set some values
        cache_storage.set("key1", vec![1, 2, 3], None).await.unwrap();
        cache_storage.set("key2", vec![4, 5, 6], None).await.unwrap();
