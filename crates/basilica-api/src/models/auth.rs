...
impl AuthConfig {
    ...
    pub fn validate(&self) -> Result<(), AuthError> {
        ...
        if self.auth_endpoint.is_empty() {
            return Err(AuthError::ConfigError("Auth endpoint is required".to_string()));
        }
        
        if self.token_endpoint.is_empty() {
            return Err(AuthError::ConfigError("Token endpoint is required".to_string()));
        }
        
        if self.redirect_uri.is_empty() {
            return Err(AuthError::ConfigError("Redirect URI is required".to_string()));
        }
        
        // Validate URLs
        if !self.auth_endpoint.starts_with("http://") && !self.auth_endpoint.starts_with("https://") {
            return Err(AuthError::ConfigError("Auth endpoint must be a valid URL".to_string()));
        }
        
        if !self.token_endpoint.starts_with("http://") && !self.token_endpoint.starts_with("https://") {
            return Err(AuthError::ConfigError("Token endpoint must be a valid URL".to_string()));
        }
...
mod tests {
    ...
    fn test_auth0_config() {
        let config = AuthConfig::auth0_default(
            "example.auth0.com".to_string(),
            "client123".to_string(),
        );
        
        assert_eq!(config.auth_endpoint, "https://example.auth0.com/authorize");
        assert_eq!(config.token_endpoint, "https://example.auth0.com/oauth/token");
