    ...
    pub fn matches_executor(&self, executor: &Executor) -> bool {
        ...
        if self.gpu_count > 0 {
            ...
            if !self.gpu_types.is_empty() {
                let executor_gpu_types: Vec<String> =
                    executor.gpu_specs.iter().map(|g| g.name.clone()).collect();
...
impl Executor {
    ...
    pub fn display_info(&self) -> String {
        let gpu_info = if self.gpu_specs.is_empty() {
            ...
            format!(
                "{} ({}GB)",
                self.gpu_specs[0].name, self.gpu_specs[0].memory_gb
            )
        } else {
            format!(
                "{}x {} ({}GB each)",
        ...
        format!(
            ...
            self.id, self.cpu_specs.cores, self.memory_gb, gpu_info
        )
    }
}
...
impl ExecutorFilters {
    ...
    pub fn matches(&self, executor: &Executor) -> bool {
        ...
        if let Some(min_gpu_memory) = self.min_gpu_memory {
            if executor.gpu_specs.is_empty() || executor.gpu_specs[0].memory_gb < min_gpu_memory {
                return false;
            }
        }
