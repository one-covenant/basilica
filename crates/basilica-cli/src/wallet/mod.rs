...
pub fn get_wallet_addresses(
    ...
    let coldkey = wallet.get_coldkeypub(password_string.clone())
        .ok()
        .and_then(|keypair| keypair.ss58_address());
    
    // Try to get hotkey address  
    let hotkey = wallet.get_hotkeypub(password_string)
        .ok()
        .and_then(|keypair| keypair.ss58_address());
...
pub fn wallet_exists(base_wallet_path: &Path, wallet_name: &str) -> bool {
    ...
    let hotkey_path = base_wallet_path.join(wallet_name).join("hotkeys").join("default");
