...
use super::types::{AuthResult, TokenSet, AuthError};
...
impl TokenStore {
    ...
    pub fn new() -> AuthResult<Self> {
        let project_dirs = ProjectDirs::from("ai", "basilica", "basilica-cli")
            .ok_or_else(|| AuthError::StorageError("Could not determine config directory".to_string()))?;
    ...
    async fn store_in_keychain(&self, service_name: &str, tokens: &TokenSet) -> AuthResult<()> {
        let _entry = self.keyring_entry.as_ref().ok_or_else(|| {
            AuthError::StorageError("Keyring entry not available".to_string())
        })?;
        
        // Serialize tokens to JSON
        let tokens_json = serde_json::to_string(tokens).map_err(|e| {
            AuthError::StorageError(format!("Failed to serialize tokens: {}", e))
        })?;
    ...
    async fn retrieve_from_keychain(&self, service_name: &str) -> AuthResult<Option<TokenSet>> {
        ...
        match service_entry.get_password() {
            ...
            Err(e) => Err(AuthError::StorageError(format!("Failed to retrieve tokens from keychain: {}", e))),
        }
    }
    ...
    async fn delete_from_keychain(&self, service_name: &str) -> AuthResult<()> {
        ...
        match service_entry.delete_password() {
            ...
            Err(e) => {
                return Err(AuthError::StorageError(format!("Failed to delete tokens from keychain: {}", e)));
            }
        }
    ...
    async fn store_in_file(&self, service_name: &str, tokens: &TokenSet) -> AuthResult<()> {
        let storage_path = self.storage_path.as_ref().ok_or_else(|| {
            AuthError::StorageError("Storage path not configured".to_string())
        })?;
        
        let file_path = storage_path.join(format!("{}.json", service_name));
        
        let tokens_json = serde_json::to_string_pretty(tokens).map_err(|e| {
            AuthError::StorageError(format!("Failed to serialize tokens: {}", e))
        })?;
    ...
    fn load_metadata(&self) -> AuthResult<TokenMetadata> {
        ...
        let metadata_json = fs::read_to_string(&self.metadata_path).map_err(|e| {
            AuthError::StorageError(format!("Failed to read metadata: {}", e))
        })?;
    ...
    fn save_metadata(&self, metadata: &TokenMetadata) -> AuthResult<()> {
        let metadata_json = serde_json::to_string_pretty(metadata).map_err(|e| {
            AuthError::StorageError(format!("Failed to serialize metadata: {}", e))
        })?;
        
        fs::write(&self.metadata_path, metadata_json).map_err(|e| {
            AuthError::StorageError(format!("Failed to write metadata: {}", e))
        })?;
    ...
    fn update_metadata_for_service(&self, service_name: &str, storage_key: &str) -> AuthResult<()> {
        ...
        metadata.services.insert(service_name.to_string(), storage_key.to_string());
