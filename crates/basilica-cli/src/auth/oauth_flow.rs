...
impl OAuthFlow {
    ...
    pub fn new(config: AuthConfig) -> Self {
        debug!("Initializing OAuth flow with client_id: {}", config.client_id);
        Self {
            config,
            code_verifier: None,
            code_challenge: None,
    ...
    pub async fn start_flow(&mut self) -> AuthResult<TokenSet> {
        ...
        let expected_state = self.state.as_ref()
        ...
        let code = callback_data.code
            .ok_or_else(|| AuthError::CallbackServerError("No authorization code received".to_string()))?;
