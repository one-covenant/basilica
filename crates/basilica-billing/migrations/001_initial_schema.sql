...
-- Billing packages
CREATE TABLE billing.billing_packages (
...
CREATE TABLE billing.package_gpu_rates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE billing.user_packages (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES billing.users(user_id),
...
-- Indexes
CREATE INDEX idx_credits_user_id ON billing.credits(user_id);
...
CREATE INDEX idx_credit_transactions_reference ON billing.credit_transactions(reference_id, reference_type);
CREATE INDEX idx_credit_reservations_user_id ON billing.credit_reservations(user_id);
CREATE INDEX idx_credit_reservations_rental_id ON billing.credit_reservations(rental_id);
CREATE INDEX idx_credit_reservations_status ON billing.credit_reservations(status);
CREATE INDEX idx_user_packages_user_id ON billing.user_packages(user_id);
CREATE INDEX idx_user_packages_active ON billing.user_packages(user_id, is_active)
WHERE is_active = true;
CREATE INDEX idx_billing_rules_package_id ON billing.billing_rules(package_id);
CREATE INDEX idx_billing_rules_active ON billing.billing_rules(is_active)
WHERE is_active = true;
