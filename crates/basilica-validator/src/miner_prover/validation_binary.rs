...
impl BinaryValidator {
    ...
    /// Clean up any zombie SSH tunnels from previous runs
    fn cleanup_zombie_tunnels(&self) {
        ...
        let output = match tokio::time::timeout(timeout_duration, child.wait_with_output()).await {
            ...
            Err(_) => {
                ...
                if let Some(pid) = child_pid {
                    ...
                    if let Err(e) = ProcessTerminator::terminate(
                        pid as i32,
...
"#;
    ...
    async fn test_process_group_creation() {
        ...
        // Clean up
        let _ = ProcessTerminator::terminate(
            pid as i32,
    ...
    async fn test_process_termination() {
        ...
        let result = ProcessTerminator::terminate(
            pid as i32,
            std::time::Duration::from_secs(1),
        )
        .await;

        // Process termination might fail if the process already exited, which is fine
        if let Err(e) = &result {
            eprintln!("Note: Process termination returned error (process may have already exited): {}", e);
        }
