...
//     let matcher = async |exec: &&ExecutorConfig, query: &str, db: &RegistrationDb| -> bool {
//         let executor_id = db
//             .get_or_create_executor_id(&exec.grpc_address)
//             .await
//             .expect("should already exist");
//         // Try exact match first
//         let id = executor_id.uuid.to_string();
//         if id == query {
//             return true;
//         }
...
//             let display_fn = async |e: &&ExecutorConfig, db: &RegistrationDb| {
//                 let executor_id = db
...
pub fn format_executor_display(
    ...
    if let Some(identity) = &executor.identity {
        ...
        output.push_str(&format!("{:<20}", executor.config.grpc_address));
    }
