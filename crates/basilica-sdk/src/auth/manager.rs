...
impl TokenManager {
    ...
    pub fn new(provider: Box<dyn AuthProvider>, storage_key: impl Into<String>) -> AuthResult<Self> {
        Ok(Self {
            provider,
    ...
    pub async fn get_access_token(&self) -> AuthResult<String> {
        ...
        info!("No valid tokens available, authenticating with {}", self.provider.name());
    ...
    pub async fn revoke(&self) -> AuthResult<()> {
        if let Some(token_set) = self.storage.retrieve(&self.storage_key).await? {
            ...
            let token = token_set.refresh_token.as_ref()
